...
void main() {
  ...
  group('Rounded calculation', () {
    ...
    test('bmi should be calculated and rounded', () {
      final bmi = TrefethenBmiCalculator().calculateBmiPrecision2(170, 70);
      expect(bmi, 24.15);
    });

    test('bmi should not be calculated and rounded as weight is null', () {
      final bmi = TrefethenBmiCalculator().calculateBmiPrecision2(170, 0);
      expect(bmi, null);
    });

    test('bmi should not be calculated and rounded as height is null', () {
      final bmi = TrefethenBmiCalculator().calculateBmiPrecision2(0, 70);
      expect(bmi, null);
    });

    test('bmi should not be calculated and rounded as params are null', () {
      final bmi = TrefethenBmiCalculator().calculateBmiPrecision2(0, 0);
      expect(bmi, null);
    });
  ...
  group('Calculation without rounding', () {
    ...
    test('bmi should be calculated', () {
      final bmi = TrefethenBmiCalculator().calculateBmi(170, 70);
      expect(bmi, 24.15010864536703);
    });

    test('bmi should not be calculated as weight is null', () {
      final bmi = TrefethenBmiCalculator().calculateBmi(170, 0);
      expect(bmi, null);
    });

    test('bmi should not be calculated as height is null', () {
      final bmi = TrefethenBmiCalculator().calculateBmi(0, 70);
      expect(bmi, null);
    });

    test('bmi should not be calculated as params are null', () {
      final bmi = TrefethenBmiCalculator().calculateBmi(0, 0);
      expect(bmi, null);
    });
  });
