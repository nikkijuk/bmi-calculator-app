...
- CalculatorHome's body property is filled with BlocProvider for CalculatorBloc, CalculatorPage is child property
...
- at CalculatorPage *S.of(context)?.height* does NOT work, and thus some texts are not shown
- this problem is to be seen during BDD test, with *flutter run* all texts are shown correctly according to selected locale
